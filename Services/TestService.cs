...
namespace GrpcTest
{
    ...
    {
        public override Task<HttpResponse> IsOdd( HttpRequest request, ServerCallContext context)
        {

            try
            {
                if (Convert.ToBoolean(Convert.ToInt32(request.Num) % 2))
                    return Task.FromResult(new HttpResponse { Message = "奇数" });
                else
                    return Task.FromResult(new HttpResponse { Message = "偶数" });
            }
            catch
            {
                return Task.FromResult(new HttpResponse { Message = "非整数" });
            }
        }
        /// <summary>
        /// 将一个对象转换为指定类型
